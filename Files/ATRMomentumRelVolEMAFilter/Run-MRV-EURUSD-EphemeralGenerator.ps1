param(
  ...
  [string]$ExperimentId = 'mrv_ema_eg_eurusd_2025_is36m_val3m_oos3m_step1m',
  [string]$Symbol = 'EURUSD',
  [string]$Period = 'H1',
  [string]$TemplateSetFile = 'ATRMomentumRelVolEMAFilter_WalkForward_Current.set',
  [string]$TempSetFile = 'ATRMomentumRelVolEMAFilter_Generated_Current.set',
  [string]$TempConfigPath = (Join-Path $PSScriptRoot 'mrv_ema_ephemeral_generator_current.ini'),
...
function New-TestSetFile {
  ...
  Set-InputLine -Lines $lines -Name 'g_MomentumATRMultiplier' -Value $Test.MomentumATRMultiplier
  ...
  Set-InputLine -Lines $lines -Name 'g_RelVolSignalCandles' -Value (Get-PropValue -Object $Test -Names @('RelVolSignalCandles') -Default 1)
  Set-InputLine -Lines $lines -Name 'g_RelVolThreshold' -Value (Get-PropValue -Object $Test -Names @('RelVolThreshold') -Default 1.5)
  Set-InputLine -Lines $lines -Name 'g_FastEMALength' -Value $Test.FastEMALength
  Set-InputLine -Lines $lines -Name 'g_SlowEMALength' -Value $Test.SlowEMALength
  Set-InputLine -Lines $lines -Name 'g_MinEMASeparationCandles' -Value $Test.MinEMASeparationCandles
  Set-InputLine -Lines $lines -Name 'g_RiskPercentOfBalance' -Value $Test.RiskPercentOfBalance
  Set-InputLine -Lines $lines -Name 'g_TakeProfitSLMultiple' -Value $Test.TakeProfitSLMultiple
  Set-InputLine -Lines $lines -Name 'g_MaxStopLossATRMultiple' -Value $Test.MaxStopLossATRMultiple
...
function New-OptimizerCandidates {
  ...
  foreach ($is in $OptimizerRows) {
    ...
    [pscustomobject]@{
      ...
      MomentumATRMultiplier = [double](Get-PropValue -Object $is -Names @('g_MomentumATRMultiplier') -Default 0.0)
      ...
      RelVolSignalCandles = [int](Get-PropValue -Object $is -Names @('g_RelVolSignalCandles') -Default 1)
      RelVolThreshold = [double](Get-PropValue -Object $is -Names @('g_RelVolThreshold') -Default 1.5)
      FastEMALength = [int](Get-PropValue -Object $is -Names @('g_FastEMALength') -Default 100)
      SlowEMALength = [int](Get-PropValue -Object $is -Names @('g_SlowEMALength') -Default 400)
      MinEMASeparationCandles = [int](Get-PropValue -Object $is -Names @('g_MinEMASeparationCandles') -Default 0)
      RiskPercentOfBalance = [double](Get-PropValue -Object $is -Names @('g_RiskPercentOfBalance') -Default 1.0)
      TakeProfitSLMultiple = [double](Get-PropValue -Object $is -Names @('g_TakeProfitSLMultiple') -Default 0.0)
      MaxStopLossATRMultiple = [double](Get-PropValue -Object $is -Names @('g_MaxStopLossATRMultiple') -Default 3.0)
    }
  }
}
...
function New-ValidationManifest {
  ...
  foreach ($candidate in $Candidates) {
    ...
    $rows.Add([pscustomobject]@{
      ...
      ATR = $candidate.ATR
      MomentumATRMultiplier = $candidate.MomentumATRMultiplier
      ContiguousCandles = $candidate.ContiguousCandles
      RelVolLength = Get-PropValue -Object $candidate -Names @('RelVolLength') -Default 20
      RelVolSignalCandles = Get-PropValue -Object $candidate -Names @('RelVolSignalCandles') -Default 1
      RelVolThreshold = Get-PropValue -Object $candidate -Names @('RelVolThreshold') -Default 1.5
      FastEMALength = $candidate.FastEMALength
      SlowEMALength = $candidate.SlowEMALength
      MinEMASeparationCandles = $candidate.MinEMASeparationCandles
      RiskPercentOfBalance = $candidate.RiskPercentOfBalance
      TakeProfitSLMultiple = $candidate.TakeProfitSLMultiple
      MaxStopLossATRMultiple = $candidate.MaxStopLossATRMultiple
...
function New-OosManifest {
  ...
  foreach ($periodDef in $periods) {
    ...
    $rows.Add([pscustomobject]@{
      ...
      RelVolSignalCandles = Get-PropValue -Object $Selected -Names @('RelVolSignalCandles') -Default 1
...
function Invoke-TestManifest {
  ...
  foreach ($test in ($Manifest | Sort-Object { [int]$_.TestIndex })) {
    ...
    $configContent = @(
      ...
      "Report=$($test.Report)",
      '',
      '; Fixed run generated by MRV EMA ephemeral generator.',
      '',
      'ShutdownTerminal=1'
    )
...
function Read-ManifestReportRows {
  ...
  foreach ($test in $Manifest) {
    ...
    [pscustomobject]@{
      ...
      ReportPath = $path
      ATR = $test.ATR
      MomentumATRMultiplier = $test.MomentumATRMultiplier
      ContiguousCandles = $test.ContiguousCandles
      RelVolLength = Get-PropValue -Object $test -Names @('RelVolLength') -Default 20
      RelVolSignalCandles = Get-PropValue -Object $test -Names @('RelVolSignalCandles') -Default 1
      RelVolThreshold = Get-PropValue -Object $test -Names @('RelVolThreshold') -Default 1.5
      FastEMALength = $test.FastEMALength
      SlowEMALength = $test.SlowEMALength
      MinEMASeparationCandles = $test.MinEMASeparationCandles
      RiskPercentOfBalance = $test.RiskPercentOfBalance
      TakeProfitSLMultiple = $test.TakeProfitSLMultiple
      MaxStopLossATRMultiple = $test.MaxStopLossATRMultiple
    }
  }
}
...
function Export-WindowSelection {
  ...
  $row = [pscustomobject]@{
    ...
    RelVolSignalCandles = if ($null -ne $Selected) { Get-PropValue -Object $Selected -Names @('RelVolSignalCandles') -Default 1 } else { '' }
...
if ($PrepareOnly) {
  ...
  if ($firstRunnable.Count -gt 0) {
    ...
    New-OptimizerConfig -Window $firstRunnable[0] -ConfigPath (Join-Path $PSScriptRoot 'mrv_ema_ephemeral_optimizer_current.ini') -Report $optimizerReport -TemplateSetFileName $TemplateSetFile
  }
...
foreach ($window in ($windows | Sort-Object { [int]$_.WindowIndex })) {
  ...
  $optimizerConfigPath = Join-Path $PSScriptRoot 'mrv_ema_ephemeral_optimizer_current.ini'
