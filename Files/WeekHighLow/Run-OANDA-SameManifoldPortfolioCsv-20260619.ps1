...
$progressPath = Join-Path $reportDir 'portfolio_csv_replay_progress.csv'
$manifestPath = Join-Path $reportDir 'portfolio_csv_replay_manifest.csv'
...
Copy-Item -LiteralPath $PSCommandPath -Destination (Join-Path $reportDir 'Run-OANDA-SameManifoldPortfolioCsv-20260619.ps1') -Force

$PassId = @(Expand-FilterValues $PassId)
$Symbol = @(Expand-FilterValues $Symbol)

$availablePasses = @(
  [pscustomobject]@{ Pass = '2551'; SourceSet = 'ImpulseContinuation_OANDA_SameManifold_Pass2551.set' },
  [pscustomobject]@{ Pass = '2012'; SourceSet = 'ImpulseContinuation_OANDA_SameManifold_Pass2012.set' }
)

$selectedPasses = @($availablePasses | Where-Object { $PassId -contains $_.Pass })
if ($selectedPasses.Count -eq 0) {
  throw 'No selected passes matched available CSV replay passes.'
}
...
$testIndex = 1
foreach ($pass in $selectedPasses) {
  $manifoldId = "OANDA_SameManifold_Pass$($pass.Pass)_FullPortfolio"
  $csvFile = Join-Path $commonFilesDir "manifold_trades_$manifoldId.csv"

  foreach ($symbolName in $Symbol) {
    $testId = "${symbolName}_Pass$($pass.Pass)_FULL"
    $setFileName = New-CsvEnabledSetFile -SourceSet $pass.SourceSet -ManifoldId $manifoldId -TestId $testId
    $iniPath = New-TestConfigFile -SymbolName $symbolName -SetFileName $setFileName -ManifoldId $manifoldId -TestId $testId
...
$manifest | Select-Object Pass, ManifoldId, TestId, Symbol, CsvPath | Format-Table -AutoSize
