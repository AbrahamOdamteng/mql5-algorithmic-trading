...
Score-gated reselection diagnostics were added on `2026-08-03` for the same four completed EURUSD windows. The runner now reads MT5 forward spreadsheet `Back Result` and `Forward Result` columns, exposes `-MinBackScore` and `-MinForwardScore`, and can rank candidates after score gates by validation DD, profit, validation trades, total back-plus-forward trades, or profit factor. When saying "highest profit" in this runner, the current implementation means highest forward/validation profit; back/IS profit is not part of that ranking except through the score gate. `BackForwardScoreThenHighestTotalTrades` ranks score-gated candidates by `ISTrades + ValidationTrades`.
...
- `Profiles/Tester/ATRMomentumRelVolEMAFilter_WalkForward_Current.set`: optimizer preset for the experimental EMA-filtered variant. As of `2026-08-06`, the narrowed optimizer ranges are ATR period `24 -> 240` step `24`, momentum ATR multiplier `2.0 -> 7.0` step `0.5`, contiguous candles `1 -> 10` step `1`, relative-volume length `10 -> 40` step `5`, relative-volume signal candles `1 -> 10` step `1`, relative-volume threshold `1.0 -> 10.0` step `0.5`, fast EMA `25 -> 300` step `25`, slow EMA `100 -> 800` step `50`, minimum EMA separation candles `0 -> 24` step `3`, take-profit multiple `1.0 -> 3.0` step `0.5`, and max stop-loss ATR multiple `1.0 -> 10.0` step `0.5`. `g_RiskPercentOfBalance` is fixed at `1.0` and no longer optimized in this preset.
