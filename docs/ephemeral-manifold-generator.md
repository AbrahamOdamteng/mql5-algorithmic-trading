...
Speed diagnostic on `2026-08-06`: a separate process ID, `mrv_ema_fwd_eurusd_2025_is24m_val24m_oos1m_step1m_optm1ohlc`, tested MT5 `Model=1` (`1 minute OHLC`) for optimizer+forward runs while keeping fixed OOS tests on real ticks. The run covered `17` EURUSD monthly OOS starts from `2025.01.01` through `2026.05.01`, averaging `28.98` minutes per optimizer+forward window. Results failed promotion: selected windows `13 / 17`, no-selection windows `4`, profitable selected windows `2 / 13`, zero-trade selected windows `2`, net OOS `-4,482.18`, `62` OOS trades, and worst OOS DD `3.99%`. The OHLC optimizer selected different candidates from the real-tick run, for example W0001 selected `MRV_EMA_Pass5157` and lost `-950.69` OOS versus the earlier real-tick W0001 `MRV_EMA_Pass3285` at `+5,218.80`. Treat `Model=1` as a failed drop-in acceleration diagnostic, not as a replacement for real-tick optimizer evidence.

Process-stability risk surfaced by the OHLC diagnostic: if repeated real-tick genetic optimizer runs for the same window also produce different selected candidates, then one completed generator run is only one stochastic sample. Before promotion, run explicit same-window repeatability tests under separate experiment IDs and compare selected candidates, validation metrics, and real-tick OOS distributions.

Risk and margin note: earlier MRV EMA diagnostics optimized `g_RiskPercentOfBalance`, but the current optimizer preset fixes it at `1.0` to reduce the signal-search space. Some selected manifolds may still take back-to-back trades, and the intended deployment is multi-symbol, where each symbol can initiate a trade once per hour. Single-symbol test reports do not model all symbols competing for the same account margin. FTMO-style leverage around `100x` and OANDA personal-account leverage around `30x` imply different feasible risk levels, especially for high-margin symbols such as metals and indices. Future generator scoring or portfolio replay should model shared used margin, free-margin floor, symbol contract specifications, leverage assumptions, overlapping positions, and exposure caps before treating any selected portfolio as deployable.

XAUUSD follow-up note: the same strict score-gated process was partially tested on `XAUUSD`. W0001 selected `MRV_EMA_Pass8410` but produced `0` OOS trades, and W0002 had no selection. The first two optimizer+forward windows took about `21.5` hours total, so do not extend XAUUSD blindly before resolving runtime, selection stability, and portfolio-complementarity questions.
