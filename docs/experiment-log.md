...
### 2026-06-24 - EURUSD Trade-Direction Genetic Review

- Goal: Test the newly added `g_TradeDirectionMode` input by allowing the optimizer to choose continuation mode (`0`) or reversal mode (`1`) in a EURUSD genetic optimizer-forward run.
- Source folder: terminal-data `reports/eurusd_trade_direction_genetic_20260623`.
- Setup files: `Files/WeekHighLow/EURUSD_TradeDirection_Genetic_20260623.ini`, `Files/WeekHighLow/Run-EURUSD-TradeDirection-Genetic-20260623.ps1`, and `Profiles/Tester/ImpulseContinuation_EURUSD_TradeDirection_Genetic_20260623.set`.
- Test setup: EURUSD, `H1` tester timeframe, real ticks, `2000.01.01 -> 2018.01.01`, optimizer-forward validation enabled, D1 high/low stop-loss-split logic, CSV logging disabled, and `g_TradeDirectionMode=0||0||1||1||Y`.
- Run status: User ran the MT5 test. Optimizer and forward XML files were completed in the dedicated result subdirectory.
- Overall parse result: `4,447` optimizer/forward paired rows, `2,361` positive IS+forward pairs, `934` ratio-qualified pairs, `908` after the `30%` drawdown cap, `311` strict trade-count candidates, and `847` loose trade-count candidates.
- Trade-direction result: Continuation mode (`0`) dominated the run with `907` drawdown-capped candidates, `311` strict candidates, and `847` loose candidates. Reversal mode (`1`) produced only `1` drawdown-capped candidate and `0` strict or loose candidates.
- Reversal note: The only reversal survivor was pass `660`, with IS profit `4,174.50`, forward profit `7,180.41`, min ratio `2.225`, but only `1` IS trade and `3` forward trades, so it is not meaningful.
- Best new candidate: pass `2984`, continuation mode, combined IS+forward profit `716,691.40`, min ratio `13.906`, IS profit `408,573.74`, IS DD `29.38%`, IS trades `592`, forward profit `308,117.66`, forward DD `19.59%`, and forward trades `291`.
- Pass `2984` parameters: `g_ATR_Period=42`, `g_MinClusterSize=8`, `g_ATR_Cluster_multiplier=0.5`, `g_ATR_StopLoss_multiplier=0.4`, `g_impulse_lookback_hours=96`, `g_pullback_lookforward_hours=24`, `g_Impulse_ATR_multiplier=0.4`, `g_MinPullback_ATR_multiplier=0.6`, `g_TakeProfitMultiplier=3`, and `g_TradeDirectionMode=0`.
- Strict-candidate convergence: Strict survivors all used continuation mode. Common regions were high `g_MinClusterSize` (`8 -> 10` common), `g_ATR_Cluster_multiplier=0.5`, stop-loss multiplier mostly `0.3 -> 0.5`, impulse lookback often `168`, pullback lookforward often `30`, impulse multiplier often `0.4`, pullback multiplier often `0.4 -> 0.8`, and take profit most often `1`.
- Comparison to `OANDA-EURXAU-P2012`: Pass `2984` is stronger than P2012 on EURUSD optimizer-forward metrics only. P2012 source optimizer-forward metrics were combined profit `672,364.63`, min ratio `3.10`, IS DD `19.62%`, forward DD `21.45%`, IS trades `731`, and forward trades `336`.
- Deployment interpretation: No new candidate is better than `OANDA-EURXAU-P2012` as an overall deployment candidate yet, because the new candidates have only optimizer-forward evidence. P2012 still has broader validation across EURUSD OOS, XAUUSD transfer, portfolio replay, shifted windows, execution-noise Monte Carlo, floating/equity DD review, and live `10K` risk sizing.
- Decision or next step: Do not pursue reversal mode from this run. If challenging P2012, run fixed OOS and same-manifold `EURUSD + XAUUSD` transfer tests for pass `2984`, with optional backup shortlist `2041`, `3998`, `3786`, and `2188`.

### 2026-06-24 - P2012 FX28 Trade Duration Profitability Analysis

- Goal: Determine whether trade duration has a measurable relationship with profitability using the existing `28`-symbol P2012 FX IS+VAL trade CSV.
- Source trade CSV: common-files `manifold_trades_OANDA_EURXAU_P2012_FX28_ISVAL.csv`.
- Source report folder: terminal-data `reports/oanda_pass2012_fx28_isval_20260623`.
- Method: Paired `IN` and `OUT` rows by `test_id + trade_id`, calculated duration from entry `deal_time` to final exit `deal_time`, summed exit profit/swap/commission as closed net P/L, and grouped results by duration bin, decile, segment, and symbol.
- CSV health: `50,141` raw rows, `25,065` paired closed trades, `11` unmatched entry rows ignored, `0` unmatched exit rows, and trade rows for `27 / 28` FX symbols.
- Output files created: `p2012_fx28_trade_duration_closed_trades.csv`, `p2012_fx28_trade_duration_bin_summary.csv`, `p2012_fx28_trade_duration_decile_summary.csv`, `p2012_fx28_trade_duration_symbol_summary.csv`, `p2012_fx28_trade_duration_segment_summary.csv`, and `p2012_fx28_trade_duration_correlation_summary.csv`.
- Overall result: Across FX28 IS+VAL, total closed net was `-1,680,171.31` with overall win rate `28.43%`, matching the earlier conclusion that P2012 does not transfer broadly across FX28.
- Correlation result: Pearson duration-vs-net-profit correlation was `0.1259`, Pearson log-duration-vs-net-profit was `0.1630`, Spearman duration-vs-net-profit was `0.1501`, Pearson duration-vs-win/loss was `0.1989`, and Pearson log-duration-vs-win/loss was `0.2704`.
- Winner/loser duration result: Winning trades lasted materially longer than losing trades. Winners had median duration `10.10h`, average duration `20.75h`, and p90 duration `61.85h`. Losers had median duration `2.68h`, average duration `10.29h`, and p90 duration `26.20h`.
- Duration-bin result: Trades closing within `<=1h` were strongly loss-making with `6,498` trades, win rate `12.76%`, total net `-2,215,409.46`, and profit factor `0.363`. Trades closing within `1 -> 4h` were also strongly loss-making with `6,029` trades, win rate `20.00%`, total net `-1,623,749.65`, and profit factor `0.529`.
- Duration-bin result: Trades lasting `4 -> 12h` were slightly profitable with total net `166,633.53` and profit factor `1.063`. Trades lasting `12 -> 24h` were strongly profitable with total net `972,125.40` and profit factor `1.670`. Trades lasting `1 -> 3d` produced total net `682,703.03` and profit factor `1.530`. Trades lasting `3 -> 7d` produced total net `313,683.47` and profit factor `1.841`.
- Decile result: Profitability improved monotonically from the shortest duration deciles into the middle/longer deciles. The first five duration deciles were net negative; decile `6` was near breakeven; deciles `7 -> 10` were profitable.
- Within-symbol check: The positive duration/profit relationship was not only caused by EURUSD. All `27` symbols with trades had positive duration-vs-net and duration-vs-win correlations, although strength varied by symbol.
- EURUSD-specific result: EURUSD trades closing within `<=1h` lost `-33,237.14` with profit factor `0.875`; `1 -> 4h` was near breakeven at `4,426.12`; `4 -> 12h`, `12 -> 24h`, and `1 -> 3d` were strongly profitable with profit factors `1.876`, `1.603`, and `1.895` respectively.
- Practical interpretation: Fast stop-outs are the main damage source in the FX28 transfer run, while trades that survive past the first few hours are much healthier. Duration is mostly known only after entry, so this is not directly an entry filter. It is more useful for exit-management research, early-failure diagnostics, or deciding whether time-based stop/exit rules are worth testing.
- Decision or next step: Do not treat duration alone as a deployable filter. If exploring this further, test time-based management rules on a frozen shortlist, such as avoiding or modifying trades that remain open less than `4h`, but require fixed OOS validation because any duration-based rule can easily overfit after observing the completed trade outcome.

### 2026-07-18 - ThreeDayTrendSignal EURUSD H1 Momentum Genetic Review

- Goal: Review the first genetic optimization of `Experts/ThreeDayTrendSignal/ThreeDayTrendSignalEA.mq5` after adding market-order execution from newly drawn ATR momentum circles.
- Setup files: `Files/ThreeDayTrendSignal/TDTS_EURUSD_Genetic_20260718.ini` and `Profiles/Tester/ThreeDayTrendSignal_EURUSD_Genetic_20260718.set`.
- Result files: terminal-data `reports/three_day_trend_signal_eurusd_genetic_20260718/TDTS_EURUSD_H1_Genetic_2000_2018_FWD_20260718.xml` and matching `.forward.xml`.
- Test definition: `EURUSD`, `H1`, `2000.01.01 -> 2018.01.01`, MT5 genetic optimization enabled, forward mode enabled, `0.10` fixed lots per trade, no percentage-risk sizing.
- Supersession note: after this review, `ThreeDayTrendSignalEA.mq5` was changed to fixed starting-balance risk sizing using `g_StartingBalance` and `g_RiskPercentOfBalance`. These fixed-lot genetic rankings are useful as a preliminary signal only and should be rerun under the current risk-sizing model before candidate promotion.
- Optimized inputs: `g_ATR_Period`, `g_ATR_Multiplier`, `g_ContiguousCandles`, `g_StopLossATRMultiple`, and `g_TakeProfitSLMultiple`.
- Row counts: `1541` optimizer rows, `1541` forward rows, `1541` paired rows, `1053` forward-profitable rows, `772` rows profitable in both optimizer and forward, and `148` rows with profit/DD ratio above `2.0` in both optimizer and forward.
- Important caveat: Some highest forward-profit passes were weak or negative in-sample. Do not promote by forward profit alone.
- Rejected forward outliers: pass `844` had forward profit `9806.03` but weak IS ratio `0.128`; passes `215`, `216`, `900`, and `549` had negative IS profit despite strong forward profit.
- Best balanced promotion candidates: pass `962` with IS `7996.70`, IS DD `0.9531%`, `948` IS trades, forward `3328.88`, forward DD `0.5787%`, `728` forward trades, parameters `ATR=49`, `ATRMult=3.5`, `Candles=3`, `SL_ATR=1.75`, `TPxSL=1.5`; pass `1070` with IS `7390.09`, IS DD `0.9585%`, `864` IS trades, forward `3524.53`, forward DD `0.6947%`, `698` forward trades, parameters `ATR=42`, `ATRMult=3.0`, `Candles=2`, `SL_ATR=1.75`, `TPxSL=1.5`; pass `1168` with IS `4288.78`, IS DD `0.7777%`, `868` IS trades, forward `2832.20`, forward DD `0.4266%`, `651` forward trades, parameters `ATR=35`, `ATRMult=3.5`, `Candles=3`, `SL_ATR=1.75`, `TPxSL=0.75`.
- Additional high-forward-profit candidates worth fixed testing: pass `1062` with IS `4288.49`, IS DD `1.7496%`, `478` IS trades, forward `6019.21`, forward DD `0.8584%`, `420` forward trades, parameters `ATR=42`, `ATRMult=2.5`, `Candles=1`, `SL_ATR=2.25`, `TPxSL=4.75`; pass `33` with IS `4596.70`, IS DD `1.4006%`, `478` IS trades, forward `5670.11`, forward DD `0.9252%`, `420` forward trades, parameters `ATR=42`, `ATRMult=2.5`, `Candles=1`, `SL_ATR=2.0`, `TPxSL=4.75`.
- High-stability lower-profit candidates: pass `1456` with forward `921.58`, forward DD `0.1308%`, `219` forward trades, parameters `ATR=21`, `ATRMult=3.0`, `Candles=1`, `SL_ATR=1.75`, `TPxSL=0.5`; pass `1170` with forward `1970.52`, forward DD `0.3016%`, `543` forward trades, parameters `ATR=28`, `ATRMult=3.0`, `Candles=2`, `SL_ATR=1.75`, `TPxSL=0.75`; pass `1040` with forward `1048.26`, forward DD `0.1323%`, `219` forward trades, parameters `ATR=21`, `ATRMult=3.0`, `Candles=1`, `SL_ATR=2.0`, `TPxSL=0.5`.
- Decision or next step at time of review: passes `962`, `1070`, `1168`, `1062`, and `33` were the best fixed-lot candidates. Because risk sizing changed afterward, rerun the EURUSD genetic optimization with `g_StartingBalance=100000.0` and `g_RiskPercentOfBalance=1.0` before fixed OOS promotion.
