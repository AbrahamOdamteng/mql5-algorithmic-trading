...
Latest MRV EMA variant note: `Experts/ATRMomentumRelVolEMAFilter/ATRMomentumRelVolEMAFilterEA.mq5` was created as an experimental A/B EA with fast/slow EMA chart lines and EMA-gated square-marker trades. It compiled cleanly and visually looked correct, but the 3-window EURUSD 2025 throwaway-manifold diagnostic `mrv_ema_eg_eurusd_2025_is36m_val3m_oos3m_step1m` should be treated as a failed diagnostic, not a promotable strategy. `PositiveBestRatio` was the only practical selector, producing `+50,327.77` net OOS, `3 / 3` profitable windows, worst DD `25.23%`, and `85` trades, but profit was concentrated almost entirely in W0001 while W0002-W0003 were barely positive. Other modes were mostly negative or had extreme drawdown; `HighestDD` made `+69,145.13` but with worst DD `68.73%` and only `2 / 3` profitable windows. If revisited, constrain/simplify the EMA search space and invalid EMA combinations before more OOS testing.
