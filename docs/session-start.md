...
Current MRV EMA preset note: `Profiles/Tester/ATRMomentumRelVolEMAFilter_WalkForward_Current.set` was narrowed on `2026-08-06` to reduce optimizer state space. Key changes: ATR period `24 -> 240` step `24`, momentum ATR multiplier `2.0 -> 7.0` step `0.5`, RelVol length `10 -> 40` step `5`, fast EMA `25 -> 300` step `25`, slow EMA `100 -> 800` step `50`, minimum EMA separation `0 -> 24` step `3`, take-profit multiple `1.0 -> 3.0` step `0.5`, and fixed `g_RiskPercentOfBalance=1.0` with optimization disabled.

Latest GBPUSD slim MRV EMA note: W0001 was tested under `mrv_ema_fwd_gbpusd_2025_is24m_val24m_oos1m_step1m_slim_totaltrades01` using `BackForwardScoreThenHighestTotalTrades` with `BackScore >= 90` and `ForwardScore >= 90`. It selected `MRV_EMA_Pass14873`, with `BackScore=98.44`, `ForwardScore=92.72`, `177` total IS+validation trades, validation profit `43,882.17`, and validation DD `10.14%`, but OOS `2025.01.01 -> 2025.02.01` lost `-1,564.16`, DD `4.13%`, and `4` trades. The prior `BackForwardScoreThenHighestTrades` GBPUSD W0001 run selected `MRV_EMA_Pass4009` and lost `-2,066.48`; a fresh repeat selected no candidate. Treat GBPUSD W0001 as a failed OOS datapoint and evidence that optimizer repeatability remains unresolved.
