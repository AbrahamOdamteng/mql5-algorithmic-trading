...
- `g_RiskPercentOfBalance` remains optimizer-enabled intentionally in the MRV EMA process. It is not only a return knob: it is a crude margin/capacity-sizing parameter because some manifolds can take back-to-back trades, and the planned portfolio can have multiple symbols initiating trades in the same hour. Single-symbol tester runs unrealistically give one symbol access to the full account margin, while live FTMO-style deployment has shared portfolio margin, typically `100x` leverage, and OANDA personal-account deployment may have only `30x` leverage. Later validation needs portfolio-level margin replay or stress testing before any selected manifolds are considered deployable.
