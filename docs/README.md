...
- `Profiles/Tester/ATRMomentumRelVolEMAFilter_WalkForward_Current.set` is the matching optimizer preset. As of `2026-08-06`, its search space was narrowed: ATR period `24 -> 240` step `24`, momentum ATR multiplier `2.0 -> 7.0` step `0.5`, RelVol length `10 -> 40` step `5`, fast EMA `25 -> 300` step `25`, slow EMA `100 -> 800` step `50`, minimum EMA separation `0 -> 24` step `3`, take-profit multiple `1.0 -> 3.0` step `0.5`, and fixed `g_RiskPercentOfBalance=1.0` with optimization disabled.
...
- Score-gated reselection modes were added to the EMA forward-validation runner on `2026-08-03`. Important parser finding: MT5 forward spreadsheets expose `Forward Result` and `Back Result`, not plain `Result`, so score gates must read those columns. The wrapper supports `-MinBackScore`, `-MinForwardScore`, and modes such as `BackForwardScoreThenHighestTrades`, `BackForwardScoreThenHighestTotalTrades`, `BackForwardScoreThenHighestProfit`, `BackForwardScoreThenLowestDD`, `BackForwardScoreThenHighestDD`, `BackForwardScoreThenLowestTrades`, and `BackForwardScoreThenHighestPF`. `BackForwardScoreThenHighestTotalTrades` requires both score gates, then ranks by `ISTrades + ValidationTrades`.
- Latest four-window EURUSD reselection finding: `BackScore >= 90`, `ForwardScore >= 90`, then highest validation trades is the current lead among tested score-gated modes, with net OOS `+6,134.56`, `2 / 4` profitable windows, `1 / 4` zero-trade window, `13` OOS trades, and worst OOS DD `1.81%`. `90/90` highest validation profit produced net `+5,184.96`; `90/90` highest validation DD produced net `+5,601.09`; `90/90` lowest validation DD produced net `+1,207.59`; `90/90` lowest validation trades produced net `+1,399.06`; `90/90` highest validation PF produced net `-493.73`. `80/80` lowest-DD selected the same candidates as `90/90` lowest-DD. `80/80` highest-profit produced net `+3,915.11`, `0 / 4` zero-trade windows, `9` OOS trades, and worst DD `1.39%`.
- Latest six-window EURUSD extension for the same `90/90` highest-validation-trades rule: W0005 produced no selection because no candidate passed both score gates, while W0006 selected `MRV_EMA_Pass2975` and produced OOS `+1,518.76`, DD `1.25%`, and `3` trades. Clean six-window aggregate is net OOS `+7,653.32`, selected windows `5 / 6`, `3` profitable selected windows, `1` losing selected window, `1` zero-trade selected window, `1` no-selection window, `16` OOS trades, and worst OOS DD `1.81%`.
- Latest GBPUSD slim-preset diagnostic: W0001 under `mrv_ema_fwd_gbpusd_2025_is24m_val24m_oos1m_step1m_slim_totaltrades01` used `BackForwardScoreThenHighestTotalTrades` with `90/90` score gates. It selected `MRV_EMA_Pass14873` with `BackScore=98.44`, `ForwardScore=92.72`, `177` total back-plus-forward trades, validation profit `43,882.17`, and validation DD `10.14%`, but OOS `2025.01.01 -> 2025.02.01` lost `-1,564.16` with `4` trades and `4.13%` equity DD. Treat it as a failed single-window OOS datapoint; total-trades selection did not solve GBPUSD W0001.
- `2026-08-06` speed diagnostic: `mrv_ema_fwd_eurusd_2025_is24m_val24m_oos1m_step1m_optm1ohlc` used MT5 `Model=1` (`1 minute OHLC`) for optimizer+forward runs and a separate `_optm1ohlc` report folder so the original EURUSD real-tick reports were not overwritten. It completed `17` windows quickly, averaging `28.98` minutes per optimizer+forward window, but failed as a drop-in acceleration method: selected windows `13 / 17`, no-selection windows `4`, profitable selected OOS windows `2 / 13`, net OOS `-4,482.18`, `62` trades, and worst OOS DD `3.99%`. W0001 selected `MRV_EMA_Pass5157` and lost `-950.69` OOS, while the earlier real-tick six-window run selected `MRV_EMA_Pass3285` and made `+5,218.80`. Treat this as evidence that tester model changes can change selected manifolds, not as promotion evidence.
- New MRV EMA process risk: if repeated real-tick runs of the same window also select different candidates, then single-run generator results are stochastic samples rather than stable process measurements. Before promotion, run an optimizer-repeatability diagnostic on one or more exact same windows using separate experiment IDs.
- Earlier MRV EMA diagnostics optimized `g_RiskPercentOfBalance`, so their selected candidates still need fixed-risk review. The current MRV EMA optimizer preset fixes `g_RiskPercentOfBalance=1.0` to reduce the signal-search space; later validation still needs portfolio-level margin replay or stress testing because live FTMO-style deployment has shared portfolio margin, typically `100x` leverage, and OANDA personal-account deployment may have only `30x` leverage.
- XAUUSD MRV EMA note: the first two strict forward-validation windows were weak and expensive. W0001 selected a candidate but produced `0` OOS trades, while W0002 had no selection after about `21.5` total optimizer hours. Do not extend XAUUSD blindly before resolving runtime, selection stability, and portfolio-complementarity questions.
...
Latest EURUSD generator finding: for the first six windows of `tdts_eg_eurusd_2017_is36m_val3m_oos3m_step1m`, `PositiveLowestTrades` is the current best validation selection mode with net OOS `+9,214.41`, `3 / 6` profitable windows, worst DD `22.65%`, and `465` OOS trades. `PositiveLowestTradesFinalMonth` was close but weaker at net OOS `+7,881.33`, while `PositiveLowestTradesHardGates` and `PositiveLowestTradesQualityFloor` were worse. The key conclusion is that W0004-W0006 remain weak across modes, so simple validation-to-OOS ranking changes are likely not solving the root problem. Next useful work should test process-level changes such as 1-month OOS deployment, abstention/regime filters, different validation/deployment windows, completing daily trend/final signal logic, or trying another symbol.
