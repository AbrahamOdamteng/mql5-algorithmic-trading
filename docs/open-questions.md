...
23. MRV EMA risk-search decision: as of `2026-08-06`, `g_RiskPercentOfBalance` is fixed at `1.0` in `Profiles/Tester/ATRMomentumRelVolEMAFilter_WalkForward_Current.set` and is no longer optimized during signal discovery. Future work still needs a separate portfolio-level risk/margin feasibility sweep.
